...
public class CustomerApi {
	...
	private boolean update(@PathVariable("citizenNumber") String citizenNumber,
		...
		if (!customerSaveDto.getCitizenNumber().equals(citizenNumber)) {
			throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Citizen number cannjot be changed");
		}
	...
	private CustomerListDto getInfo(String citizenNumber) {
		Optional<Customer> existingCustomer = customerDao.findByCitizenNumber(citizenNumber);
		if (existingCustomer.isEmpty()) {
			throw new ResponseStatusException(HttpStatus.NOT_FOUND, "Given citizen number not found " + citizenNumber);
		}
