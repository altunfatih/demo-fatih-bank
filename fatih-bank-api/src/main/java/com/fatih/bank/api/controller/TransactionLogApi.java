...
import com.fatih.bank.api.dto.ExchangeTransactionDto;
import com.fatih.bank.api.dto.SingleAccountTransactionDto;
import com.fatih.bank.api.dto.TransactionLogDto;
import com.fatih.bank.api.dto.TransferTransactionDto;
import com.fatih.bank.api.helper.ModelValidationHelper;
import com.fatih.bank.db.dao.AccountDao;
import com.fatih.bank.db.dao.impl.ExchangeRateDaoImpl;
import com.fatih.bank.db.dao.impl.TransactionLogDaoImpl;
...
public class TransactionLogApi {
	...
	public TransactionLogDto buyExchange(@RequestBody @Valid ExchangeTransactionDto exchangeRequest) {
		...
		if (!account.getCurrency().equals(AccountCurrency.TL)) {
			throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
					"Exchange transactions are only allowed for TRY accounts. Given account currency: "
							+ account.getCurrency());
		}
		...
		ExchangeRate exchangeRate = exchangeRateDao.findByDateAndCurrency(LocalDate.now(),
				exchangeRequest.getCurrency());
